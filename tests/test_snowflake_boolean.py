...
def test_snowflake_boolean_columns_equal_with_nulls(snowflake_session):
    ...
    actual = columns_equal(
        dataframe,
        "LEFT_VALUE",
        "RIGHT_VALUE",
        "ACTUAL",
    ).select("ACTUAL").toPandas()["ACTUAL"]
    expected = dataframe.select("EXPECTED").toPandas()["EXPECTED"]

    assert actual.tolist() == expected.tolist()


def test_snowflake_boolean_numeric_cross_type_in_both_directions(snowflake_session):
    dataframe = snowflake_session.createDataFrame(
        [
            (True, 1, True),
            (False, 0, True),
    ...
    forward = columns_equal(
        dataframe,
        "BOOLEAN_VALUE",
        "INTEGER_VALUE",
        "FORWARD_MATCH",
    ).select("FORWARD_MATCH").toPandas()["FORWARD_MATCH"]
    reverse = columns_equal(
        dataframe,
        "INTEGER_VALUE",
        "BOOLEAN_VALUE",
        "REVERSE_MATCH",
    ).select("REVERSE_MATCH").toPandas()["REVERSE_MATCH"]
    expected = dataframe.select("EXPECTED").toPandas()["EXPECTED"]
...
def test_snowflake_boolean_comparator_ignores_non_boolean_columns(snowflake_session):
    dataframe = snowflake_session.createDataFrame(
