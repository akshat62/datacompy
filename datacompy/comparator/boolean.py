...
class SparkBooleanComparator(BaseComparator):
    ...
    @staticmethod
    def _normalized_boolean(column: str) -> Any:
        """Map Boolean values to 1/0 while preserving nulls."""
        return (
            psf.when(psf.col(column).isNull(), psf.lit(None).cast("double"))
            .when(psf.col(column), psf.lit(1.0))
            .otherwise(psf.lit(0.0))
        )
    ...
    def compare(
        ...
        if (col1_is_bool and dtype2_is_numeric) or (col2_is_bool and dtype1_is_numeric):
            left = (
                self._normalized_boolean(col1)
                if col1_is_bool
                else psf.col(col1).cast("double")
            )
            right = (
                self._normalized_boolean(col2)
                if col2_is_bool
                else psf.col(col2).cast("double")
            )
...
class SnowflakeBooleanComparator(BaseComparator):
    ...
    def _normalized_boolean(column: str) -> Any:
        """Map Boolean values to 1/0 while preserving nulls."""
        return (
            spf.when(spf.is_null(spf.col(column)), spf.lit(None).cast("double"))
            .when(spf.col(column), spf.lit(1.0))
            .otherwise(spf.lit(0.0))
        )
    ...
    def compare(
        ...
        **kwargs: Any,
        ...
        elif (col1_is_bool and dtype2.startswith(_NUMERIC_SNOWFLAKE_TYPES)) or (
            col2_is_bool and dtype1.startswith(_NUMERIC_SNOWFLAKE_TYPES)
        ):
            left = (
                self._normalized_boolean(col1)
                if col1_is_bool
                else spf.col(col1).cast("double")
            )
            right = (
                self._normalized_boolean(col2)
                if col2_is_bool
                else spf.col(col2).cast("double")
            )
            expression = left.eqNullSafe(right)
        else:
